...
function Jasmine2HTMLReporter(options) {
    ...
    self.jasmineDone = function() {
		...
		var summeryHTML = '<header> <ul class="stats"> <li>Total Tests: <strong>' + totalTests + 
						  '</strong></li> <li>Total Passed: <strong>'  + totalPassedTests + 
						  '</strong></li> <li>Total Failures: <strong>' + totalFailedTests + 
						  '</strong></li> </ul> </header>';
		...
		if (output == '') {
			...
			html += '<header>';
			html += '<h2> No failures found in this test suit! <p> &#x1F642 </p></h2>';
			html += '</header>';
			html = html + summeryHTML;
			html += '\n </article>';
			
			output = html;
		} else {
			output = output + summeryHTML;
		}
    ...
    function suiteAsHtml(suite) {
		...
		totalTests += suite._specs.length;
		totalFailedTests +=suite._failures;
		totalPassedTests = totalTests - totalFailedTests;
